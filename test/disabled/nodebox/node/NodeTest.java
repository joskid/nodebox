...
public class NodeTest {
    ...
    private final FunctionRepository mathRepository = FunctionRepository.of(MathFunctions.LIBRARY);
    ...
    public void testAddChild() {
        ...
        Node child3 = Node.ROOT.withName("child3");
        parent = parent.withChildAdded(child3);
        assertEquals(3, parent.getChildren().size());
        assertTrue(parent.hasChild("child3"));

        assertFalse(parent.hasChild("foo"));
        assertFalse(child1.hasChild("child2"));
    }

    @Test(expected = IllegalArgumentException.class)
    public void testAddChildWithSameName() {
        Node anotherChild1 = Node.ROOT.withName("child1");
        parent.withChildAdded(anotherChild1);
    }

    @Test
    public void testRemoveChild() {
        parent = parent.withChildRemoved("child1");
    ...
    public void testRemoveNonChild() {
        parent.withChildRemoved("foo");
    }

    @Test
    public void testRenderedChild() {
        assertEquals("By default the rendered child name is null.", "", parent.getRenderedChildName());
        assertNull("By default the rendered child is null.", parent.getRenderedChild());

        parent = parent.withRenderedChildName("child1");
        assertSame(child1, parent.getRenderedChild());

        parent = parent.withRenderedChildName("child2");
        assertSame(child2, parent.getRenderedChild());

        parent = parent.withRenderedChildName("");
        assertNull(parent.getRenderedChild());
    }

    @Test(expected = NullPointerException.class)
    public void testRenderedChildNonNull() {
        parent.withRenderedChildName(null);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testUnknownRenderedChild() {
        parent.withRenderedChildName("foo");
    }
    ...
    public void testForFunction() {
        ...
        assertEquals("int1", pInt1.getName());
        assertEquals(0, pInt1.getValue());

        Port float1 = ports.get(1);
        assertEquals(Port.TYPE_FLOAT, float1.getType());
        assertEquals("float1", float1.getName());
        assertEquals(0f, float1.getValue());

        Port string1 = ports.get(2);
        assertEquals(Port.TYPE_STRING, string1.getType());
        assertEquals("string1", string1.getName());
        assertEquals("", string1.getValue());

        Port point1 = ports.get(3);
        assertEquals(Port.TYPE_POINT, point1.getType());
        assertEquals("point1", point1.getName());
        assertSame(Point.ZERO, point1.getValue());

        assertEquals("0, 0.0, , Point(0.0, 0.0)", render(testRepository, allTypesNode));
    }
    ...
    public void testError() {
        ...
        // We just changed a port value, so the node is dirty.
