...
public class NodeLibraryTest extends TestCase {
    ...
    /**
     * Test to check if a node where a port has a value set but in its prototype
    ...
    public void testLoadingOverriddenExpression() {
        ...
        // Perform the same check for a port without expression but whose parent have an expression set,
    ...
    public void testExternalDependencies() {
        ...
        // After setting the external dependency on the port, triggering the dependency makes the node dirty.
        ...
        n.update();
        assertFalse(n.isDirty());
        // After removing all external dependencies on the port,
    ...
    public void testFrameDependency() {
        ...
        Node n = root.create(Node.ROOT_NODE, "n");
        Parameter p = n.addParameter("p", Parameter.Type.FLOAT);
        // Setting the expression to frame declares an external dependency on the port.
    ...
    public void testCanvasDependency() {
        ...
        n.setRendered();
        Parameter p = n.addParameter("p", Parameter.Type.FLOAT);
        // Setting the expression to WIDTH declares an external dependency on the port.
    ...
    /**
     * Assert that the given text can be used as the description for a port.
    ...
    /**
     * Assert that the given value can be stored as a port value in a NodeBox script.
