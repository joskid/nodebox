...
public class ParameterTest extends NodeTestCase {
    ...
    public void testNaming() {
        ...
        n.addPort("myport");
        assertInvalidName(n, "myport", "port names must be unique across parameters and ports");
    }
    ...
    public void testInvalidName() {
        ...
        assertInvalidName(pBob, "alice", "Can not take the name of an existing port.");
    ...
    /**
     * Test if all port attributes are cloned.
     */
    ...
    /**
     * Test if all port attributes are copied.
     */
    ...
    /**
     * Test if changes to the port value fire the correct event.
     */
    public void testParameterValueEvents() {
        ...
        // Initialization has triggered the port value event.
    ...
    /**
     * Test if changes to the port metadata fire the correct event.
     */
    ...
    public void testInternalType() {
        Node alpha = Node.ROOT_NODE.newInstance(testLibrary, "alpha");
        Parameter pFloat = alpha.addParameter("float", Parameter.Type.FLOAT);
        // We can set a double or integer value to a float port.
        pFloat.set(33);
        ...
        alpha.update();
        // Since this is a float port, getValue should always return a float.
    ...
    public void testRevertToDefault() {
        // Create a prototype node with an int port with an expression.
        ...
        // Revert to default. The beta port should have a regular value instead of an expression.
    ...
    /**
     * Test if the port needs to be marked dirty every time we ask for upstream data.
     */
    public void testHasStampExpression() {
        ...
        n.update();
        assertTrue(pAlpha.hasStampExpression());
        // Set the port to the *same* expression.
        pAlpha.setExpression("stamp(\"myalpha\", 12)");
        assertTrue(pAlpha.hasStampExpression());
    ...
    public void testEnabled() {
        ...
        // Changing the dependent port will change the disabled state.
        pBeta.set(10);
        assertTrue(pAlpha.isEnabled());
        // Create a syntax error. This re-enables the port.
    ...
    private void assertParameterNotFound(Node n, String parameterName) {
        assertNull("The port \"" + parameterName + "\" should not have been found.", n.getParameter(parameterName));
    }
