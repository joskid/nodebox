...
public class MenuControl extends AbstractPortControl implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        nodebox.node.MenuItem item = (nodebox.node.MenuItem) menuBox.getSelectedItem();
        String value = item.getKey();
        if (!port.getValue().equals(value)) {
            port.setValue(item.getKey());
        }
    }

    private class MenuDataModel implements ComboBoxModel {

        java.util.List<nodebox.node.MenuItem> menuItems;
        nodebox.node.MenuItem selectedItem;

        public MenuDataModel(Port port) {
            menuItems = port.getMenuItems();
        }

        public nodebox.node.MenuItem getMenuItem(String key) {
            for (nodebox.node.MenuItem item : menuItems) {
        ...
        public void setSelectedItem(Object anItem) {
            selectedItem = (nodebox.node.MenuItem) anItem;
        }
